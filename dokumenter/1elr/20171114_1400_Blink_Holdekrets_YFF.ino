//#########################################################################################################################################################################################################################################
//#########################################################################################################################################################################################################################################
//#################   Eksempelprogram for en digital holdekrets skrevet i programmerinsspråket c i Arduino for elever fra Strømmen og Bleiker vgs.  #######################################################################################
//#################   Dato: 25 okt. 2017                                                                                                            #######################################################################################
//#################   Forfatter: James Fox                                                                                                          #######################################################################################
//#################   Sted: Kuben vgs.                                                                                                              #######################################################################################
...
//Deklarering- og Initialiseringsseksjon (skape variabler som inneholder verdier)
//_________________________________________________________________________________________________________________________________________________________________________________________________________________________________________
 //Bool
  boolean bMinneBitBryterHoldekrets = false; // Deklarerer en variabel med navn bMinneBitBryterHoldekrets og initialiserer (gir den en startverdi) med verdien LOW=false=0
  boolean bKnappTilstand= false; // Deklarerer en variabel med navn bKnappTilstand og initialiserer (gir den en startverdi) med verdien LOW=false=0
 ...
 //int-variabl som skal brukes til Innganger
  const int diKnapp = 2; // Deklarerer en konstant variabel(kan ikke endres senere) med navn diKnapp av datatypen int (heltall) og initialiserer (gir den en startverdi) hvor vi setter den lik 3
 //int-variabl som skal brukes til Utganger
...
void setup() 
{
  ...
  // Outputs
  pinMode(doHoldeKretsLED, OUTPUT); //LED'en som viser selve holdekretsen er koblet til utgang 3
}
...
void loop() 
{
  bKnappTilstand = digitalRead(diKnapp); // Tilstanden(sjekk av inngang) til trykknappen skrives til et minnebit (av datatypen bool som kun lagrer binære verdier: true=HIGH=1 og false=LOW=0). Vi lager et minnebit 
                                         //av flere årsaker, deriblandt for å "fryse verdien" til trykknappen når den går gjennom resten av instruksjonene i loopen. (Logikken vil bli feil om en slipper knappen midtveis i instruksjonene)
  if ( (bKnappTilstand == HIGH) && (bForrigeLoopKnappTilstand == LOW) ) // Veivalg med bruk av if-betingelse: Om minnebit'et til trykknappen er lik høy OG (AND) forrige verdi til trykknappen var lav i forrige runde i loopen, da går vi inn i IF-setningen (vi har 
                                                                        //altså akkurat trykket inn knappen der den nettopp har gått fra lav til høy)
  {
  ...
  delay(20); /*enkel måte (litt primitiv) å fjernere "debouncing" på -rett og slett tilbakeslag som oppstår når det oppstår mange små raske gnister i det en mekanisk kontakt lukkes/sluttes
